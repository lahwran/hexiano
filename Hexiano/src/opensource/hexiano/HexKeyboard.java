...
public class HexKeyboard extends View
{
    ...
    static HashMap<Integer,ArrayList<HexKey>> mKeysById = new HashMap<>();
    static HashMap<Integer,ArrayList<HexKey>> mKeysByWrappedId = new HashMap<>();
    ...
    */
    ...
    private void addkey(HexKey k) {
        mKeys.add(k);
        ArrayList<HexKey> keys_ = mKeysById.get(k.mMidiNoteNumber);
        if (keys_ == null) {
            keys_ = new ArrayList<>();
            mKeysById.put(k.mMidiNoteNumber, keys_);
        }
        keys_.add(k);

        keys_ = mKeysByWrappedId.get((k.mMidiNoteNumber + 1000) % 12);
        if (keys_ == null) {
            keys_ = new ArrayList<>();
            mKeysByWrappedId.put((k.mMidiNoteNumber + 1000) % 12, keys_);
        }
        keys_.add(k);
    }
    ...
    public boolean onTouchEvent(MotionEvent event){
        ...
        Set<Integer> new_notes = new HashSet<Integer>();
        for (Integer kid : new_pressed) {
            int i = mKeys.get(kid).mMidiNoteNumber;
            new_notes.add(i);

            ArrayList<HexKey> surrogates = mKeysById.get(i);
            ArrayList<HexKey> octaves = mKeysByWrappedId.get((i + 1000) % 12);
            if (surrogates != null) {
                for (HexKey sk : surrogates) {
                    sk.setSurrogatePressed(true);
                }
            }
            if (octaves != null) {
                for (HexKey ok : octaves) {
                    ok.setOctavePressed(true);
                }
            }
        }

        Set<Integer> just_released = new HashSet<Integer>(old_pressed);
        Set<Integer> just_released_notes = new HashSet<Integer>();
        just_released.removeAll(new_pressed);
        it = just_released.iterator();
        while (it.hasNext()) {
            int i = it.next();
            HexKey k = mKeys.get(i);
            k.stop();
            int m = k.mMidiNoteNumber;
            if (!new_notes.contains(m)) {
                just_released_notes.add(m);
            }
            //this.invalidate();
        }
        for (Integer i : just_released_notes) {
            ArrayList<HexKey> surrogates = mKeysById.get(i);
            ArrayList<HexKey> octaves = mKeysByWrappedId.get((i + 1000) % 12);
            if (surrogates != null) {
                for (HexKey sk : surrogates) {
                    sk.setSurrogatePressed(false);
                }
            }
            if (octaves != null) {
                for (HexKey ok : octaves) {
                    ok.setOctavePressed(false);
                }
            }
        }
