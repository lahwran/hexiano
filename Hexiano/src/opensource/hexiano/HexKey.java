...
public abstract class HexKey
{
    ...
    static int mOctaveColor;
    static int mSurrogateColor;
    ...
    Paint mSurrogatePressPaint = new Paint();
    Paint mOctavePressPaint = new Paint();
    ...
    private boolean mSurrogatePressed;
    private boolean mOctavePressed;
    ...
    public HexKey(Context context, int radius, Point center, int midiNoteNumber, Instrument instrument, int keyNumber)
    {
        ...
        mStreamId = new int[] {-1};
        mPressed = false;
        mSurrogatePressed = false;
        mOctavePressed = false;
        mDirty = true;
        mRadius = radius;
        mCenter = center;
        ...
        mSurrogatePressPaint.setColor(mSurrogateColor);
        mSurrogatePressPaint.setAntiAlias(true);
        mSurrogatePressPaint.setStyle(Paint.Style.FILL);
        mSurrogatePressPaint.setStrokeWidth(2);

        mOctavePressPaint.setColor(mOctaveColor);
        mOctavePressPaint.setAntiAlias(true);
        mOctavePressPaint.setStyle(Paint.Style.FILL);
        mOctavePressPaint.setStrokeWidth(2);
    ...
    protected void setColors()
    {
        ...
        mSurrogateColor = 0xFFA98949;
        //mSurrogateColor = 0xFFFF0000;
        mOctaveColor    = 0xFF39F959;
        //mOctaveColor    = 0xFFFF00FF;
    }
    ...
    public void setSurrogatePressed(boolean p) {
        if (mSurrogatePressed == p) {
            return;
        }
        mSurrogatePressed = p;
        mDirty = true;
    ...
    public void setOctavePressed(boolean p) {
        if (mOctavePressed == p) {
            return;
        }
        mOctavePressed = p;
    ...
    public void paint(Canvas canvas)
    {
        ...
        else if (mPressed || mSurrogatePressed || mOctavePressed)
        {
            // Key shaping/painting
            hexPath.offset(mCenter.x, mCenter.y);
            canvas.drawPath(hexPath, mPressed ? mPressPaint : (mSurrogatePressed ? mSurrogatePressPaint : mOctavePressPaint)); // Background (greyed) color
    ...
    public void play(float pressure)
    {
        ...
        if (mStreamId[0] != -1) {
            ...
            if (!HexKeyboard.mSustainAlwaysOn) {
